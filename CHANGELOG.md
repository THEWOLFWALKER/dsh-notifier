...
- **QQ attachments 图片/文件闭环（#36：附件消息此前被静默丢弃）**（`src/inbound/message.mjs`、`src/inbound/qq-gw.mjs`、`src/host/messages.mjs`、`src/inbound/conversation.mjs`、`src/routing/task-selection.mjs`、`src/strings.mjs`）：依据腾讯官方 C2C / 群 @ 消息事件的 `attachments` 段（`docs/protocol-preflight/qq-bot.md` 事件白名单已登记，真机样本未核验——contract-tested，不宣称 real-device-verified），新增 `parseQqAttachments` 逐项白名单归一：`content_type` 以 `image/` 开头走图片（复用 `normalizeImageAttachment`），其余类型走文件（`normalizeFileAttachment`，需显式 http(s) URL，文件名经新增 `sanitizeFileName` 剥路径分量/C0-C1 控制字符并按码点截断到 128）。QQ 网关 C2C 与群 @ 两条事件路径均改收 `attachments`（图片缺失时回落既有 `extra` 过渡段），纯附件用 `[图片消息]`/`[文件消息]` 占位并随信封带 `attachments[]`。投递链（`conversation.mjs`）新增 `collectAttachments` 统一归一（规范形状 `attachments[]` + 过渡期单项 `image`/`file`，同 kind 不重复 admission），`deliver` 逐项「有界下载实读字节（新增 `downloadInboundFileBytes`：与图片同一套 SSRF/`redirect:error`/有限超时口径，文件不限 Content-Type，5 MiB 统一上限）→ `ctx.attachments.saveFile` durable admission → `{ type:'file', attachment: ref }` 块」，多附件保持入站顺序、逐个独立成败（部分失败不阻断其余），失败回执按失败 kind 取 `strings.conversation.{image,file,attachment}FetchFailed`（zh/en 成对），**绝不回退远程 URL 继续塞进 Session**；无 `saveFile` 能力/下载失败一律 fail-closed，纯附件全失败绝不塞空消息。任务选择（`task-selection.mjs`）待决条目附 `attachments[]`（含脱敏视图与 `sanitizeAttachments`，legacy `image` 形状兼容读）。
- 测试：新增 `test/conversation.attachment-delivery.test.mjs` 8 项 focused（text+file / image+file 顺序保持且不重复图片 / 纯文件占位剥离 / 部分失败不阻断 / 混合失败回执 / 纯附件全失败不塞空消息 / 缺 attachment service 降级 / 文件名去路径）；`test/inbound.message.test.mjs` 增 focused（`parseQqAttachments` 分派与未知字段不透传、`sanitizeFileName`、`normalizeFileAttachment`、`downloadInboundFileBytes` 实读字节上限）；`test/task-routing.test.mjs` 断言补 `attachments`。全量 `npm test` 全绿。

- **修复 Feishu inbound 交互发送缺少有限 HTTP transport timeout（#31 收口）**（`src/inbound/feishu-bot.mjs`、`test/inbound.feishu.test.mjs`、`test/feishu-p2p-source.test.mjs`、`docs/memory/risks.md`）：基于 `@larksuiteoapi/node-sdk@1.73.0` 公开的 `Client({httpInstance})` seam（npm README 明确 `httpInstance` 为 Client 构造参数、`defaultHttpInstance` 可从 SDK 导入；official SDK `typings/http.ts` 定义底层 `HttpRequestOptions.timeout?: number`），给飞书 `Client` 注入一个仅属于该 Client 的隔离 `bounded HttpInstance`（`withBoundedTimeout` + `createTimedHttpInstance`，显式绑定 request/get/delete/head/options/post/put/patch 全方法，`request()` 为 hard gate），在底层 HttpRequestOptions 层强制 `<=10s` 有限 timeout（保留更短值、非法值回落 10000），连带覆盖 message create/patch 之前的 tenant token 获取链路。不修改 SDK 共享 `defaultHttpInstance`（严禁 mutate `defaults.timeout`）、不 `Promise.race`、不盲重试（timeout 按普通 transport failure 走入既有 warn/`false`/`null`/单次文本兜底语义）；不新增公开配置面（`FEISHU_HTTP_TIMEOUT_MS` 为内部安全上限，不进 config/admin/README）；WSClient 不变。Issue #31 的 QQ / DingTalk / Feishu 三条 inbound timeout 缺口至此全部收口。
- 测试：`test/inbound.feishu.test.mjs` 扩展 fake SDK（`defaultHttpInstance` + `httpRequests` 记录 + `failHttp`），使 create/patch 经真实 transport wrapper，新增 5 项 focused（Client 注入 + 缺省 10000、更短/非法值回落 + 不 mutate + 不碰共享 defaults、sendText false/卡片 null、patch timeout 恰一次兜底 attempt）；`test/feishu-p2p-source.test.mjs` fake SDK 补齐 `defaultHttpInstance` 满足 ensureStarted 新 hard gate。focused 65 ✓，全量 npm test ✓。

- **Host P0-B：远程取消改用 structured `AgentCancelCause`（对齐 DSH v0.1.7-rc.1）**（`src/inbound/conversation.mjs`、`src/index.mjs`、`test/conversation.test.mjs`、`test/index.test.mjs`）：依据官方 deepseek-harness 实际源码——`packages/core/session/src/types.ts` 的 `AgentCancelCause` 是可辨识联合 `{kind:'user'} | {kind:'parent'} | {kind:'hook', reason:string} | {kind:'disposed'}`，`packages/core/agent-loop/src/agent.ts` 的 `ReactLoopAgent.cancel(cause)` 把 cause 直传 `phase.abort.abort(cause)`。此前 `/stop` 命令与 `turn/cancel` 动作分别传字符串 `'remote-stop'` / `'remote-action'`——既不是 DSH 契约里的合法 discriminant，也会丢失宿主 abort 侧的 cause 区分语义。现在两处远程手机用户取消统一为 `agent.cancel({ kind: 'user' })`（远程按钮触发的取消语义即用户取消）；不改 `/stop` 解析、Control Core、action token/ledger、会话选择与回执文案。
- 测试：`test/conversation.test.mjs` 三处 `/stop` 断言由 `['remote-stop']` 改为 `[{ kind:'user' }]`；`test/index.test.mjs` 新增源码级守卫（turn/cancel 装配点闭包不可外部触达，与 wiring.route 源码守卫同模式）锁 index 与 conversation 两处装配点不再传字符串 cause、均传 `{ kind:'user' }`。全量 `npm test` 全绿。

- **Host P0-A：入站图片走 durable image block，淘汰 `image_url` / `plugin` 来源（对齐 DSH v0.1.7-rc.1 UserMessage V4）**（`src/host/messages.mjs` 新增、`src/inbound/conversation.mjs`、`src/inbound/message.mjs`、`test/conversation.test.mjs`、`test/conversation.image-delivery.test.mjs`、`test/conversation.task-selection.test.mjs`）：依据官方 deepseek-harness 实际源码——`packages/llm/llm/src/message.ts`（无 catch-all `plugin` kind，producer 自声明 `kind`；`ContextFormed.summary` 上界 `CONTEXT_SUMMARY_MAX_CHARS=120`）、`packages/llm/llm/src/types.ts`（`ImageBlock` 为 `{ type:'image', attachment: ImageAttachmentRef }`，`image_url` 已不在 V4 契约）、`packages/attachment/attachment/src/index.ts`+`types.ts`（`ctx.attachments.saveImage({ data, mediaType }) → ImageAttachmentRef`；`ImageMediaType` 白名单 png/jpeg/webp/gif）。此前入站图片把远程 URL 塞进 OpenAI 风格 `image_url` 块、`source.kind='plugin'`，且 `downloadInboundImage` 只做可达性校验（best-effort 后台下载不落盘，图片并**未**真正交到模型）。现在 `source.kind` 收敛为 `dsh-notifier`（producer-owned）；图片先「有界下载实读字节（新增 `downloadInboundImageBytes`：上限按实读字节计、不信 Content-Length、超限立即 cancel、redirect:error）→ `ctx.attachments.saveImage` durable admission → `{ type:'image', attachment: ref }` 块」，绝不回退远程 URL（红线 2.4）；attachment service 缺失/媒体类型不在白名单/下载失败一律 fail-closed（正文照投 + 失败回执，纯图失败绝不塞空消息），text-only 不依赖 attachments。投递链随之异步化（`deliver`/`selectAndDeliver`/`handleTaskUse`/`routeUnsafe`），总线 handler 同步调用点统一经 `fireAsync` 兜底（任何 rejection 只 warn，绝不弄崩宿主/总线）。
- 测试：`test/conversation.test.mjs` 来源断言改为 `source.kind='dsh-notifier'` 且不夹带 legacy `plugin` 字段；`test/conversation.image-delivery.test.mjs` 全量改写为 durable image block 契约（attachment stub + `downloadImageBytes` 注入点：文本+图双载 / 纯图不落占位 text / 合并窗图只随一次 / 下载失败回执不阻断 / 成功以 durable ref 随文）；`test/conversation.task-selection.test.mjs` `/use` 用例补 waits 异步投递链。全量 `npm test` 1680 全绿。

- **QQ keyboard 协议收敛（#26，对齐腾讯 dsh-qqbot 参考实现）**（`src/inbound/qq-gw.mjs`、`test/inbound.qq.test.mjs`）：审批/提问按钮此前带 `permission.specify_user_ids`（一个未经证实的 provider 字段，腾讯当前 `question-renderer.ts`/`approval-renderer.ts` 均为 `permission: { type: 2 }` 不带该字段），且按钮 label 超长未做协议约束截断（提问卡 label 含选项全文）。现在 `permission` 收敛为 `{ type: 2 }`（权限仍在 Control Core 的 token/account/user/chat + pending + 一次性裁决完成，keyboard 层无需再叠一层 provider 限制），按键 label 经新增 `clampButtonLabel` 整体 ≤10 码点（按码点截断补省略号，G-22 同根不产生孤立代理项；en/zh `visited_label` 与 `customAnswerLabel` 一并经 clamp）。**不宣称 specify_user_ids 是 #26 唯一根因**——真机不渲染是上游协议/devices 范畴，本次仅移除多余字段并修正 label 协议约束。群聊不发可操作按钮、`click_limit=1`、`type=1`（回调按钮）语义不变。
- 测试：`test/inbound.qq.test.mjs` 改 2 项 + 增 1 项 focused（审批/提问按钮 `permission` 精确断言为 `{ type: 2 }` 且 `specify_user_ids` 为 undefined；超长/emoji 选项 label ≤10 码点截断、custom label 保留语义且无孤立代理项）。
...
- **turn/end 正文迁移有界缓存（去 `session.events`/`snapshotEvents`）**（`src/event-listener.mjs`、`test/event-listener.test.mjs`、`test/lang-strings.test.mjs`）：DSH 0.1.2 起 `session.events` getter 已移除（issue #32），此前 turn/end 通知正文与心跳「最近输出摘录」恒为空。现在 `assistant/message` 事件本就流经 `session/event` 总线（官方 `packages/core/session/src/types.ts` 的 `SessionEventMap['assistant/message']`，`data.message.content` 为文本块数组），趁事件到达把文本沉淀进新增的 `createAssistantTextCache`——按 sessionId 存「最近一次输出」，有界（缺省 256 条，超限淘汰最旧，宪法#4），turn/end 正文与心跳摘录改从缓存取。删除旧 `lastAssistantText(session)`（读 `session.events` 回落 `snapshotEvents()` 的两段式，宿主面已不存在，属 dead host API）；正文兜底「无正文摘要」语义不变。取舍：防抖本就按 session 合并（10s 窗），缓存不引入 turn 键——新一轮产出只会让防抖窗内的 turn/end 正文带上更新内容，与既有「最近输出摘录」口径一致。
- 测试：`test/event-listener.test.mjs` 改写为 `assistantTextOf`（单条事件提取）+ `createAssistantTextCache`（有界/淘汰/覆盖）+ 集成（总线喂 assistant/message 后 turn/end 正文携带，未喂则「无正文摘要」）；stall 心跳摘录用例与 `test/lang-strings.test.mjs` en 心跳摘录用例改为经总线喂事件（不再靠 `session.events`）。全量 1662 → 1663。
- 收口验证（2026-09-25）：`rg -n "session\.events|snapshotEvents" src` 仅命中 `src/event-listener.mjs` 的 4 处说明性注释，**生产调用 0**；focused `node --test test/event-listener.test.mjs` = 45 pass；全量 `npm test` = 1703 pass。两路径均有回归覆盖——turn/end 正文（`createEventListener #32：turn/end 正文从有界缓存取`）与 stall 心跳「最近输出摘录」（`v0.5 stall`）。本项不再写运行时代码；GitHub Issue #32 的评论与 close 动作留给 owner / 带 `gh` 凭证的会话执行（当前沙箱无 `gh`，本地仅存证据）。
- **入站 transport 有限超时**（`src/inbound/qq-gw.mjs`、`src/inbound/dingtalk-stream.mjs`、`src/inbound/feishu-bot.mjs`、`docs/memory/risks.md`）：QQ/钉钉入站换 token、发文本/卡片、互动回执、开网关、业务 POST 此前全走裸 fetchImpl（无 signal），`config.timeoutMs` 仅解析从未注入——宿主线程或网络卡死时这些调用可无限挂起。现在每个 fetch 注入 `AbortController + signal`（对齐 `src/adapters/_shared.mjs` 的 `postJson/postForm` 超时语义与 `src/inbound/telegram-bot.mjs` 的 AbortController 先例；数值归一由 resolve 层负责，缺省 10000ms）；超时 `AbortError` 由调用方既有 catch 处理（token 失败进 token 管理器传播，发送失败 warn+降级），**不盲目重试**（结果未知时重试会重复投递，`_shared.mjs` G-50 同根）。飞书走官方 SDK（`@larksuiteoapi/node-sdk@1.73.0`），Client 构造级 request 超时字段未在公开类型面（`types/index.d.ts`）确认——`timeout?` 仅存在于底层 `HttpRequestOptions`（per-request，`client.im.v1.message.create` 不暴露该 opts），因此**未注入、标「待验证」**，登记 `docs/memory/risks.md`（SDK 无显式超时时长连接仍可能无限挂起的残余风险）。
- 测试：`test/inbound-transport-timeout.test.mjs` 新增 3 项 focused（QQ token/发文本/卡片 fetch 均注入 AbortSignal 且超时按既有语义降级返回 null；钉钉 batchSend 业务 POST 与 token 换发注入 AbortSignal 且超时后 `sendText` 返回 false 不崩；钉钉开网关 fetch 注入 AbortSignal 且超时后「启动失败」warn、通道不可用不影响其他通道）。全量 1659 → 1662。
...
- **attach 机制（去 required-inject 副作用）**：不再给静态 `inject` 数组加 `userQuestions`（cordis 4.0.2 inject 为 required-only：宿主缺该服务时整插件拒绝加载、通知全哑）。改用运行时可选依赖 `ctx.inject(['userQuestions'], cb)`（匿名子插件，服务就绪才激活，不阻塞父插件）；无 `ctx.inject` 的宿主回落直连 attach（防御读保证不再炸）。附带修复 A-only 的 boot 竞态（服务晚于插件装配时错过 attach）。
- **waterfall 拦截器（真机接缝）**：`dsh-user-questions@0.1.5-rc.x` 只有 `ask()`，无 `registerProvider`——旧桥在任何真实宿主上都以 `no_register_provider` 降级。`attach()` 现按能力探测二选一：有 `registerProvider` 用之（未来宿主），否则注册 `ctx.on('user-questions/request', handler, { prepend: true, global: true })`（global 绕过 scopeTarget 作用域过滤，同 #28 教训）。拦截器语义：双面并发 + 首达结算——立即推卡（aq 桥，手机/管理台可见）与下游 GUI answerer（`next()`）race；手机先答则本侧答案上交宿主（下游输家 `catch` 兜底，绝不未处理拒绝）；GUI 先答则下游答案上交宿主，并以拦截器范围取消信号对称关闭本次开的账本条目（终稿语义见下方「迟到投递收尾」条目；早期基于 `adminPending`/`adminSettle` 的启发式清扫已被取代）。空请求或入站交互通道空表（`canDeliver`）时原样透传 `next()`——GUI-only 宿主行为与未装插件一致。`next()` 至多调用一次（哨兵）；拦截器任何异常透传下游或返回空答案，绝不吞宿主提问。`capabilities().mode` 新增 `waterfall` 值（seam 仍为 `native-event`）。
- **已知限制（宿主侧，如实声明）**：拦截器抢答后宿主 `ask()` 不 abort signal，GUI 远端卡片不会自动收起（orphan card，dsh-api-gateway 只在 client-result/signal-abort/ctx-release 时 retract）。修复属 DSH core（`ask()` settle 时 abort 链接 signal，约 10 行），插件侧文档化降级；手机侧卡片/账本不受影响。
- 测试：`test/native-questions.test.mjs` +6（抛错代理探测、waterfall attach/dispose、双面 race 两侧胜负、输家拒绝卫生、canDeliver 透传、空请求透传）；`test/host-capability.test.mjs` +2（防御读四态、抛错代理下 detectQuestionsMode）。全量 `npm test` 通过。
- **复审修复（CodeRabbit Moderate 风险三条，2026-09-17）**：(1) race 韧性——下游（GUI answerer）**先 reject**（异常 / headless 宿主 NO_PROVIDER）不再判死整场 race：派生 promise 对 rejection 转「永不 settle」，等手机/管理台作答兜底；(2) 对称收尾清扫闸——空文本请求（texts 为空）直接放弃收尾，绝不盲扫窗口内无关待决条目；(3) 生命周期——waterfall 监听器注册到 `ctx.inject` 回调的**子上下文**（`attach(subCtx)`），随 userQuestions 服务 fiber 自动撤销/重放，服务替换后自动重挂（重放前先撤作废句柄）。测试 +3（headless 下游先拒、空文本不清扫、subCtx 注册与重放）。
- **迟到投递收尾：终稿为拦截器范围取消信号**：废弃时间窗/问题文本启发清扫。waterfall 拦截器为每次拦截建 `AbortController`，把 `execContext.signal` 传入 `askQuestions`；GUI（下游 answerer）先答时 `abort()`——askQuestions 侧清 Stage 0→1 延迟推卡（pushTimer）、停升级、`bus.abandon` 终结当前账本行，多问请求的后续问题经循环头 `signal.aborted` 检查按 `stopped` 收束、不再开卡。覆盖两处真实缺陷：(a) GUI 在 web-first 窗口内先答时 `adminSettle('reject')` 会因行无 push 目标而 `no_target` 失败 → 迟到的手机卡；(b) 多问请求一次性收尾后循环新开的后续问题收不到收尾。测试：拦截器侧（signal 传入、abort 恰一次、无 adminSettle 盲扫）+ router 侧（Stage 0 abort 不迟推卡、多问 abort 后续不开）。全量 1629。
- **取消时序与终止语义（两处缺陷）**：(1) abort handler 注册先于直投启动——原实现挂在 `await runPush()` 之后，`webFirstMs=0` 的直投在途时取消不及；现注册紧随 wait 预登记，且 `runPush` 入口与 `pushQuestion` 逐目标（卡投 + 编号兜底文本）检查 `deliverySettled`，取消后不再启动/继续投递；(2) 显式 `ledger.terminate(qKey)` 先于 `bus.abandon`——`abandon` 默认 manual reason 不触发 `onAbandon`（bus.mjs 仅 `agent/disposed` 触发），此前已推卡片按 timeout 话术收尾而非 terminated；终止分支现正确命中（`reason:'terminated'` + markResolved 终止文案）。测试 +2（直投在途 abort 不拖满超时、已推卡按 terminated 收尾）。全量 1631。
- **投递取消完备性与确定性回归时序**：(1) hinted 编号话术补发补上取消闸——`pushQuestion` 的 `hintedInbound` 外层与逐目标内层在每次实际发送前复查 `isCancelled`（含 await 恢复后的下一目标），取消后不再发起任何新投递；(2) 迟到成功投递与终态和解——`runPush` 在 `deliverySettled` 退出前按行终态（terminated→terminatedText，其余→timeoutResolvedText）对迟到返回的卡片补一次 `markResolved`（wait 结算时这些卡不在 pushedTo，早先收尾没见过它们）；(3) 取消保证改为诚实表述（注释+risks）：「取消后不再发起任何新投递；在途 HTTP 不可撤回，迟到成功尽力收尾」；(4) 测试重写为确定性 deferred 时序（弃 sleep 猜序）：挂起卡在途 abort（双目标，第二目标绝不启动）、挂起返回 null 后零补发零广播、Stage-1 迟到卡按 terminated 精确话术收尾、已推卡断言精确 terminatedText（经 stringsOf().questions）。全量 1633。
- **迟到卡收尾保留真实终态（CodeRabbit 发现）**：新增 `finalTextOf(row)` 终态呈现话术单一事实源——按已决行的 `decision` 与落账数据（`answers`/`via`/`custom` 标记）还原与常规收尾分支完全一致的文案（answered+custom→answeredCustom、answered→answeredWithLabelsVia、skipped/terminated/timeout 各归其位，未知保守回落超时）；aq-text 落账补 `custom: true` 标记（行内附加字段）；runPush 迟到收尾改用该映射，不再以 timeout 兜底——A 卡已答后 B 卡迟到时，两张卡都呈现已答话术而非超时。测试 +2（choose 与 skip 两参数化：双目标挂起 B、A 经 adminSettle 桥级结算、B 迟到成功、断言每次编辑均为对应终态话术且绝不出现超时话术）。全量 1635。
- **终态呈现统一与测试加固**：`finalTextOf(row)` 成为常规收尾与迟到投递收尾共用的唯一终态话术映射（terminated/skip/自定义答/选项答/timeout 各归其位）——单一映射，无双路径漂移空间；选项答分支的结果同时改从已决行读取答案标签（自定义答分支仍从裁决信封取净化文本，两处同源）；新增自定义答回归（认证入站「答：…」路径：前缀剥离、`custom: true` 落账、两张卡均呈现 answeredCustom 话术）；测试台新增编辑完成信号（事件驱动等待），双卡断言改为精确 (chatId, messageId) 覆盖，双目标发送起点用 firstStarted/secondStarted deferred 标记（弃 sleep）。代码注释中的评审轮次编号全部移除。全量 1636。
- **直投路径不阻塞结算返回（CodeRabbit Major：`await runPush()` 拖住 `waitPromise`）**：`webFirstMs=0` 的直投此前要等 `await runPush()` 完成才进入 `await waitPromise`——卡片发送逐目标串行，任一目标挂起时，作答/超时/取消即便已结算，`askQuestions` 也回不去（拦截 abort 同样解不开直投在途）。现直投与定时器路径统一后台化（`runPush().catch(warn)`）：终态由 `cancelDelivery` 封口后续投递，迟到成功由 runPush 收尾路径按行终态和解。附带语义统一：直投推送异常不再立即以 `error` 交还桌面，改为告警后继续由 Web/超时收束（与延迟推送路径一致的 fail-closed）。测试 +1（直投双目标挂 B、`adminSettle` 结算后 `askQuestions` 立即返回、B 迟到成功后双卡按已答话术和解；对未修复代码确定性红——2s race 守卫）。全量 1637。
- **复审收尾（编号话术取消闸 Major + attachError/文档 Minor）**：(1) hinted 编号话术兜底改逐目标串行 await——原实现把全部 `sendText` 同步推入数组后 `Promise.all`，首个目标在途时取消无法阻止其余目标已同步启动，违背「逐目标发送前复查取消、await 返回后进入下一目标同样拦截」的承诺；现与卡片投递同款：每次发送前复查 `isCancelled`、上一目标返回后才决定是否发起下一目标，`hintedTargets` 仍只记录 `sendText` 成功的目标。(2) `native-questions.mjs` 两条成功路径（provider 注册成功 / waterfall 监听注册成功）清空 `attachError`——服务重建重放成功后 `capabilities()` 不再残留上一次的失败码。(3) 文档收口：waterfall 拦截器条目的 GUI 先答收尾改指终稿取消语义（拦截器范围 AbortSignal；早期 `adminPending`/`adminSettle` 启发式清扫已被取代）、v0.10.1 段「唯一公开 seam」表述加注后续 waterfall 回落。测试 +2（挂起首条编号话术 + abort → 第二目标绝不启动；失败→重挂成功双路径清空错误码；编号话术回归对未修复代码确定性红）。全量 1639。
- **Qmsg 3.0 迁移：endpoint 与目标语义（P1-A）**（`src/adapters/spec-channels.mjs`、`test/config-validation.test.mjs`、`test/fixtures/channels/qmsg.json`、`README.md`）：Qmsg 官方已升级到 3.0——端点由 `https://qmsg.zendee.cn/{type}/{key}` 改为 `POST https://qmsg.zendee.cn/v3/send/{key}`，失败描述字段由 `reason` 改为 `message`（成功判定仍以 `success === true`，官方文档明确 `code` 不可靠）。目标语义随之变化：v3 **单聊目标由 API Key 绑定的机器人好友决定**，请求参数里的单聊 `qq` / `bot` 在 v3 无等价表示，旧的 `type=send`/`type=group` 也不再是 URL 路径段。因此：① 新增可选字段 `group`（群推送群号；留空=单聊），`type`/`qq`/`bot` 三个旧键退场；② `type=group + qq=<群号>` **无损自动迁移**为 `group=<群号>`（显式 `group` 优先，旧群配置无需重配；数字形态群号同样识别）；③ 其余单聊目标语义（`type=send` / 裸 `qq` / `bot` / 未知 `type`）在**配置阶段显式报迁移错误**并给出操作指引，不静默忽略、不回落未文档化的旧 endpoint（owner 裁决 2026-09-24 #2-A）；④ 仅 `key`（无目标语义）自然归一为 v3 默认单聊。`key` 作为 path segment 整体 `encodeURIComponent`，`type` 不可能再成为任意 URL path。迁移逻辑按既有 spec 惯例抽为模块级 `migrateQmsgLegacy`（同 wps-bot `preresolve` 形态），`fields` 声明同步为 `key` + `group`，管理台不再展示已废弃键。
- 测试：`test/config-validation.test.mjs` 增 6 项 focused（v3 单聊 `body={msg}` 精确断言、v3 群聊 `body={msg,group}`、`key` 特殊字符 path 整体编码、`type=group+qq` 无损迁移 + 显式 group 优先、单聊语义四态显式拒绝 + 数字形态识别 + `type=group` 无群号与 `key` 必填、失败文案取 `message`）；`test/fixtures/channels/qmsg.json` 契约样本改为 v3 形状（`validConfig`/`invalidConfig`/`expectedRequests`/`failResponse` 同步）。全量 1703 → 1709。
- **Server酱 SC3 endpoint 纠正（P1-B）**（`src/adapters/serverchan.mjs`、`src/config.mjs`、`test/adapters.test.mjs`）：旧实现把 SC3 企业版 SENDKEY（`sctp` 前缀）送到 `https://sctp.ftqq.com/<SENDKEY>.send`——该拼法已失效（**v0.9.x 的 G-09 记录成为历史事实，此处按当前官方 SDK 纠正，不重写历史 changelog**）。现在按官方 `easychen/serverchan-sdk` 规则由 SENDKEY 派生端点：`sctp<数字>t...` → `https://<数字>.push.ft07.com/send/<SENDKEY>.send`（shard 即 key 里的数字段），Turbo 版（非 `sctp` 前缀）维持 `https://sctapi.ftqq.com/<SENDKEY>.send` 不变。派生逻辑抽为纯函数 `endpointOf(sendkey)`：`sctp` 前缀但缺 `sctp<数字>t`（如 `sctp` / `sctpfoo` / `sctp123` / `sctp123ABC` / `sctp-tABC`）在**配置阶段 fail-closed**（不落 `match()[1]` 的 TypeError），不再缓存派生端点进 resolved 状态（endpoint 完全由 key 决定，单一真相）。`key` 整体 `encodeURIComponent`，path segment 注入不可逃逸。form 体（title/desp）、响应 `code` 判定、超时、三别名优先级与 stderr 冲突提示一律不动。
- 测试：`test/adapters.test.mjs` 的 SC3 用例由旧 `sctp.ftqq.com` 期望改为精确数字子域 URL，并新增 shard 1 / shard 999 两例、malformed 七形态配置阶段拒绝（含旧的 `SCTP1234` 拼法）、Turbo 回归锚定；全部走既有 capture fetch 桩，不发真实网络。全量 1709 → 1711。
- **Host 事件订阅收敛：current ctx 优先 + 一次粘性 root 回落（去重复扩大监听）**（`src/host-events.mjs`、`test/host-events.test.mjs`、`docs/memory/risks.md`）：此前 `selectHostEventContext` 无条件优先切到文档化的 Cordis 根上下文，而 registrar 又对每次订阅传 `{ global: true }`——「root + global」两重扩大监听叠在一起，且对 current ctx 本可正常注册的宿主也绕开了插件自身的 scope 身份。现在收敛为：**以插件 current ctx 为主**（保留自身 scope 身份），仅在 current **首次注册失败**（无 `ctx.on` 或 `on` 抛错）时**一次性粘性回落**到自引用校验通过的 `ctx.root`，之后所有事件都走 root——单 registrar 内绝不逐事件漂移 target，因此 `snapshot().context` 始终等于实际注册成功的上下文；**禁止 current + root 双订阅**（同一宿主事件会投递两次，项目既有事件去重不是为重复注册兜底的）。新增 `contextCandidate`/`hostEventContexts` 纯函数承载候选选择，`selectHostEventContext` 保留为「首选目标描述」的兼容导出（不再代表 registrar 的回落逻辑）。`failures` 语义明确为「注册尝试失败次数」——current 抛错 + root 成功时为 `attempts=1 / registered=1 / failures=1`。`global: true` 语义（经 `scopeTarget` 作用域过滤）、payload 归一、`capability.detectEventsMode` 枚举（dual/current/root/unsupported）均不变。
- 测试：`test/host-events.test.mjs` 重写为 Commit12 矩阵（current 主用且不碰 root / current 抛错后 sticky 回落 root（第二事件不再试 current）/ 无 `ctx.on` 时用 root / scoped tagged 子上下文经 root 回落 / 非 Cordis root 服务拒绝 / 双缺 fail-closed / 监听器抛错仅告警 / disposer 函数透传否则 undefined / 全订阅 `global:true` / `selectHostEventContext` 首选描述）；`event-listener` + `session-registry` 回归 102 全绿。
- **原生提问 seam 口径校准 + Issue #38 技术回归（P2-B）**（`src/host/native-questions.mjs`、`src/host/capability.mjs`、`src/index.mjs` 注释、`docs/memory/project-state.md`、`docs/memory/risks.md`、`test/issue38-host-compat.test.mjs`）：**不改运行时行为**（manifest 与装配保持原样），只纠正过时口径并补 #38 回归。依据官方 deepseek-harness @ `dsh-v0.1.7-rc.1` 实际源码——`packages/interaction/user-questions/src/index.ts` 的 `UserQuestionService.ask()` 内部即 `ctx.waterfall('user-questions/request', request, noAnswerer)`（rc.1 **没有** `registerProvider()`），故正式口径改为：**`user-questions/request` waterfall 是当前正式 seam**，`registerProvider` 仅 future/legacy feature probe（`attach()` 仍先探测它再回落 waterfall，顺序不变；`detectQuestionsMode` 的 `provider-chain`/`native-event`/`unsupported` 枚举保持不改，避免破坏管理台既有展示）。同时明确工具可见性是**另一层**：官方 `ask_user_question` 由其自身 tool package（`packages/interaction/tool-ask-user`）注入 preset/TUI composition，`packages/client/ui-user-questions` 亦声明全局挂载会扩张所有 Agent tool list——「`ctx.userQuestions` 存在」**不推出**「当前 Agent 看得到官方 `ask_user_question`」，故插件自有 `ask_user` fallback 保留、不读 ToolRuntime 私有 `view/layers`。`docs/memory/project-state.md` 的「唯一公开 seam `registerProvider`」陈述同步纠正为 waterfall 当前 seam。
- 测试：新增 `test/issue38-host-compat.test.mjs` 6 项 focused——producer-owned source（unit）文本单条 / summary 截断至宿主 `CONTEXT_SUMMARY_MAX_CHARS=120` / 纯文件回落摘要且无 `image_url`；端到端（Telegram 文本信封 → Agent followup）`source.kind='dsh-notifier'`、`source.form='notice'`、无 catch-all `plugin` 键、summary 有界、content 为 V4 text 块且无 `image_url`；userQuestions 代理回归（`ctx.get(name,false)` 命中服务且**绝不**触发抛错的 `ctx.userQuestions` getter；纯抛错 Proxy 安全降级 `unsupported`）。`native-questions` / `host-capability` / `mobile-task-loop-contract` / `issue38` 回归 42 全绿。

### 新增

- **手机侧 `/log` 远程诊断日志（W6）**（`src/inbound/conversation.mjs`、`src/config.mjs`、`src/index.mjs`、`src/ledger.mjs`、`src/strings.mjs`、`test/inbound-log-command.test.mjs`、`test/ledger.test.mjs`、`test/config-validation.test.mjs`）：敏感诊断能力，**不是普通聊天命令**，四道闸缺一不可——**默认关**（`remoteLog.enabled` 仅显式 `true` 才开）、**owner-only**（`identity.list(channel)` 里同 `userId` 且 `role==='owner'`；owner 是 **channel-scoped** 绑定，同 userId 在别的渠道是 owner 不算数；identity 缺失 / `list` 抛错一律 fail-closed；member 拒绝——ledger title 可能含任务上下文）、**脱敏**（整段过 `src/redact.mjs` 正式入口 `maskSecrets`，不复制 regex）、**有界**（行数 + UTF-8 字节双上限）。数据源**单一** = `ledger.recent()`（新增只读方法：从文件尾取最新 N 条、正序返回、脏行逐条跳过、copy-on-read、绝不抛）；账本**无 sessionId 语义**，故口径如实写作「最近通知/事件摘要」，**绝不谎称「当前会话日志」**；**绝不读** `session.events` / `snapshotEvents`（宿主面不可靠，生产调用 0，静态守卫覆盖）。行投影只输出 time/kind/level/title + delivered/failed **计数**（无 source 凭据、无完整 failed error）；标题再过一遍脱敏。截断顺序 collect N → project → join → redact → line cap → byte cap；字节截断按 **code point** 递增（非 UTF-16 `slice`），truncated marker 先预留字节，保证 ≤ maxBytes 且无孤立代理项。配置 hard caps `REMOTE_LOG_HARD_MAX_LINES=200` / `REMOTE_LOG_HARD_MAX_BYTES=8192`（`REMOTE_LOG_DEFAULT_LINES=20`）——**用户不能放大**（`maxLines:100000` 也被 clamp），非有限数字回落上限（安全配置非法值不宽松解释，与 `graceSeconds` 同法）。`remoteLog.enabled` 单独开启时也建账本（否则 `/log` 恒不可用），但**晨报仍只由 `digest.enabled` 决定**（打开 /log 不会顺带开始发晨报）。disabled 文案只说「管理员需在配置中显式开启 `remoteLog.enabled`」，**不提示不存在的管理台开关**（UI 本批未实现）；owner 拒绝**不泄露谁是 owner**。`strings.mjs` 新增 `logDisabled`/`logOwnerOnly`/`logUsage`/`logEmpty`/`logSummaryHeader`/`logLine`/`logTruncated`/`logUnavailable` 并 zh/en 成对，`/help` 命令表补 `/log` 一行。身份安全**不新开第二套**——陌生人根本进不了 command（走既有 bus 准入）。
- 测试：新增 `test/inbound-log-command.test.mjs` 22 项 focused（未注入 remoteLog / `enabled:false` → disabled / owner 允许 / member 拒绝 / identity 缺失 / `list` 抛错 / 跨渠道 owner 拒绝 / 未配对不可达 / 空账本 / 一条计数 / 无参取 20 / N smaller / N above max clamp / ledger 缺 / `recent` 抛错 / 参数解析 0·负·小数·垃圾·多余参数 → usage / 标题脱敏（token-like 形态不原样出现）/ 字节上限 emoji+中文+ASCII ≤256 且 `isWellFormed` / 未超限无标记 / en 文案 / 源码守卫「remoteLog 单独开启建账本但晨报仍只由 digest.enabled 决定」/ 静态守卫零 `session.events`·`snapshotEvents` 调用）；`test/ledger.test.mjs` 增 5 项（`recent` 正序取最新 N / limit 0·负·非数 → 空 / 脏行跳过 / copy-on-read / 文件不存在 → 空）；`test/config-validation.test.mjs` 增 7 项（默认 false / 仅显式 true / clamp 上限 / 下界 / 非法值回落 / 小数取整 / 非对象形态回落）。`node --test test/inbound-log-command.test.mjs test/ledger.test.mjs test/redact.test.mjs test/config-validation.test.mjs` 77 全绿，全量 `npm test` 全绿。

- **手机侧 `/sessions` 活跃会话任务概览（W5）**（`src/inbound/conversation.mjs`、`src/strings.mjs`、`test/inbound-sessions-command.test.mjs`）：手机用户不必进管理台即可一眼看清「在忙什么 / 哪个会话待我处理 / 当前绑到哪」。数据源**单一**——与 `/tasks`、管理台同用 `projectTasks`（registry + 宿主 agent 状态折叠出的只读派生视图），**不读** `store` 的 `route:sessions` 内部键、**不新增**任何持久状态；`/sessions` 与 `/agent` 分工明确（后者偏路由调试：outbound channels / quiet / 解析链），故**不做 alias**。每行两段：「`编号. [*] workspace`」+「`sid 前缀 · 状态 · [⚠]`」——`*` 标出 `resolveTarget(envelope)` 解析出的当前绑定会话（按前缀匹配 taskRef），`⚠` 仅在 `attention === true`（待决提问/审批）时出现且**只给布尔标记、绝不显示待决内容**，sid 只显示 8 位前缀（完整 id 属敏感面），切换沿用既有 `/use`、`/bind`（footer 引导），**不新增** `/sessions use`。workspace 沿用 `projectTasks` 的 basename（不落 cwd）；状态取不到回落 strings 的 `unknown`。命令层**只生成一条完整文本、不自行分段**（超长交给发送层既有 channel segmentation）。`strings.mjs` 新增 `sessionsTitle`/`sessionsEmpty`/`sessionsCurrentMark`/`sessionsAttentionMark`/`sessionsUnknownWorkspace`/`sessionsUnknownStatus`/`sessionsRowHead`/`sessionsRowBody`/`sessionsFooter` 并 zh/en 键成对，`/help` 命令表补 `/sessions` 一行；`projectTasks` 自身逐字段降级、绝不抛，故本命令不 crash。身份安全**不新开第二套逻辑**——走既有 bus 准入（未配对用户根本触达不到命令，仅补集成测试证明）。
- 测试：新增 `test/inbound-sessions-command.test.mjs` 10 项 focused（空集合回执 / 单会话展示 workspace+sid 前缀+状态+当前绑定标记且不落完整 id / 多会话保持活跃降序 / 当前绑定仅匹配行带 `*` / attention 仅 true 行带 `⚠` / 宿主 get 抛错投影降级状态回落 unknown 不崩 / router 缺失仍列会话 / en 文案完整 / 未配对身份无回执 / 40 行长列表命令层恰一条回执且行数不丢）；`node --test test/inbound-sessions-command.test.mjs test/conversation.route.test.mjs test/task-projection.test.mjs` 42 全绿。

- **英文公共契约 `PLUGINS.en.md`（W4）**（`PLUGINS.en.md`、`PLUGINS.md`、`package.json` `files`、`scripts/verify-release.mjs`、`test/plugins-docs.test.mjs`）：给生态插件作者一份与中文 `PLUGINS.md` **结构一一对应**的完整英文契约（不是摘要）——H1/H2/H3 数量一致（13 个章节）、表格行数与代码围栏数量逐项对齐，覆盖 `ctx.notifier` 服务获取（静态 inject 唯一形态）、`version 0.7`、push API 与返回值、限流与实例预算、`dsh-notifier/sent` metadata-only 订阅及监听器军规、flush、三态语义、版本与兼容、防御式完整示例、`dsh-notifier/testing`、`dsh-notifier/types`、真机验证记录与 FAQ。两份文档顶部互加语言回链。英文版使用最新事实（Commit13 已把官方 seam 校准为 `user-questions/request` waterfall），**不含**已过时的「唯一公开 seam = registerProvider」口径。`PLUGINS.en.md` 加入 `files` 与 `verify-release` 的存在性检查。
- 测试：新增 `test/plugins-docs.test.mjs` 8 项 focused（H1 各 1 / H2+H3 数量对齐 / 13 个必需章节双语齐备 / 代码围栏数量对齐且无未闭合 / 表格行数对齐 / en 携带 0.7 与 metadata-only 与 testing+types 路径 / 双语均无 stale registerProvider 口径 / 语言回链双向）；`node --test test/plugins-docs.test.mjs` 8 全绿。
- **消费方最小示例 `examples/consumer-demo`（W3）**（`examples/consumer-demo/{package.json,README.md,src/index.mjs}`、`test/consumer-demo.test.mjs`、`scripts/verify-release.mjs`）：给生态插件作者一份可直接照抄的接入参考——静态 `inject: ['notifier']` + `ctx.notifier.push(message, { sourceName })` + `dsh-notifier/sent` metadata-only 订阅 + `dispose` 时 `flush()`，`src/index.mjs` 约 30 行。README 覆盖安装、静态 inject 唯一形态、push 返回值、`sent` 是 metadata-only 且**禁止在 handler 内二次 push**（通知回环）、`dsh-notifier/testing` fake 单测用法、`dsh-notifier/types` 类型导入。示例包 `private: true`、零依赖、ESM；**刻意不进 npm 包**——`verify-release` 新增发布边界门，`package.json files` 覆盖 `examples/` 即失败。
- 测试：新增 `test/consumer-demo.test.mjs` 5 项 focused（示例 private/零依赖/ESM / `examples/` 不被 `files` 覆盖 / 导出 DSH 契约且静态 inject / `apply` 恰一次 push 且 `sourceName` 正确并装配 `sent` 与 `dispose` / 源码级「sent handler 内绝不再 push」）；`node --test test/consumer-demo.test.mjs` 5 全绿，`npm pack --dry-run` 确认 `examples/` 未泄漏。
- **TypeScript 公共契约 `dsh-notifier/types`（W2）**（`types/index.d.ts`、`package.json` `exports`/`files`、`scripts/verify-release.mjs`、`PLUGINS.md`、`test/public-types.test.mjs`）：消费方插件作者从此可以 `import type { NotifierFacade, NotifyMessage, PushResult } from 'dsh-notifier/types'` 拿到类型安全的推送面。**不新增任何 TypeScript 依赖**（`dependencies`/`optionalDependencies`/`devDependencies` 全未动，`tsc` 不进工程），类型以手写 `.d.ts` + 纯文本契约测试落地。声明逐项对照真运行时（`src/public.mjs` / `src/notify.mjs` / `src/testing.mjs` / PLUGINS.md），只公开真实存在的字段：`NotifyMessage` 仅 `title`/`content`/`level`/`group`，`NotifyOptions` 仅 `sourceName`/`channel`（内部 notify 选项不外泄），`PushResult` 形状与每个出口分支一致，`NotifierFacade` 锁 `readonly version: '0.7'` + `push`/`flush` 签名；`SentEventRecord` 保持 **metadata-only** 隐私边界（`time`/`ok`/`delivered`/`skipped`/`failed`(脱敏)/`titleLength`/`contentLength`/`titleBytes`/`contentBytes`/`hasContent`/`source?`/`channel?`），**绝不**声明 `title`/`content`/`message`/原始错误正文；`FakeNotifier`/`FakeNotifierCall`/`FakeSimulation` 与 Commit15 的 `dsh-notifier/testing` 形状对齐。`exports` 增 type-only 子路径 `"./types": { "types": "./types/index.d.ts", "default": "./types/index.d.ts" }`，`files` 增 `types`（保证 d.ts 进包）；**刻意不设 root `"types"`**——root JS 导出面远大于 notifier 公共面，设 root 类型会误导 TS 用户误以为类型覆盖整包。`verify-release` 的公共子路径门扩展为同时校验 `./testing` 与 `./types`（`{types,default}` 对象形态按同一契约校验，目标文件必须存在且被 `files` 覆盖）。
...
- `createNativeQuestionBridge` 经宿主公开 seam `registerProvider` 把 `ctx.userQuestions` 的 `ask_user_question` 桥进现有 `aq:` 账本与 Control Core（v0.10.1 当时唯一公开 seam；v0.10 起新增 waterfall 拦截器回落，见顶部 [Unreleased]），使原生提问、Web、手机共用同一首达结算闭环。
