...
- **waterfall 拦截器（真机接缝）**：`dsh-user-questions@0.1.5-rc.x` 只有 `ask()`，无 `registerProvider`——旧桥在任何真实宿主上都以 `no_register_provider` 降级。`attach()` 现按能力探测二选一：有 `registerProvider` 用之（未来宿主），否则注册 `ctx.on('user-questions/request', handler, { prepend: true, global: true })`（global 绕过 scopeTarget 作用域过滤，同 #28 教训）。拦截器语义：双面并发 + 首达结算——立即推卡（aq 桥，手机/管理台可见）与下游 GUI answerer（`next()`）race；手机先答则本侧答案上交宿主（下游输家 `catch` 兜底，绝不未处理拒绝）；GUI 先答则下游答案上交宿主，并经公开 `adminPending`/`adminSettle(action:'reject')` 尽力对称关闭本次开的账本条目（窗口 + 问题文本启发；`adminPending` 的 agent 字段脱敏，无法按 agent 精确归属）。空请求或入站交互通道空表（`canDeliver`）时原样透传 `next()`——GUI-only 宿主行为与未装插件一致。`next()` 至多调用一次（哨兵）；拦截器任何异常透传下游或返回空答案，绝不吞宿主提问。`capabilities().mode` 新增 `waterfall` 值（seam 仍为 `native-event`）。
