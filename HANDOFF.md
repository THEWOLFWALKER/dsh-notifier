...
| 测试 | `npm test`：**1616 tests**（v0.10.1 发布收口基线） |
...
确认干净工作树、版本/测试数与 [docs/VERSIONING.md](docs/VERSIONING.md) 一致后，在 disposable DSH profile 安装已发布的 `dsh-notifier@0.10.1` registry artifact，验证启动装配、一次出站测试和一次入站命令。开发在 `dev` 分支进行、发布并入 `main`。
