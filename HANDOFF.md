...
| 测试 | `npm test`：**1625 tests**（wps-bot 出站渠道合入后基线；Windows 检出下 1621 通过，2 项为上游既有环境失败） |
...
确认干净工作树、版本/测试数与 [docs/VERSIONING.md](docs/VERSIONING.md) 一致后，在 disposable DSH profile 安装已发布的 `dsh-notifier@0.10.2` registry artifact，验证启动装配、一次出站测试和一次入站命令。开发在 `dev` 分支进行、发布并入 `main`。
