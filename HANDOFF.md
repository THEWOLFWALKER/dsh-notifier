...
> **当前开发工作线（2026-09-26）**：`v0.13.0` 已在 `dev` 完成架构收敛与发布门收口，并逐阶段推送到 `origin/dev`；npm 包已发布，本次 release closeout 完成 `main`、tag 与 GitHub Release。全量 `npm test` 为 **1984 tests / 1984 pass / 0 fail / 0 skip**，`npm run verify:release` 通过。真实 DSH 真机、provider 账号与真实投递回执未在本次收口中新增验证，继续保持 open，见 `docs/memory/risks.md`。
