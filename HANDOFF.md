...
> **当前开发工作线（2026-09-25）**：`v0.12.1` 已在 `dev` 的 `codex/v0121-durability-truthfulness` 分支完成 Phase A-E 代码/契约实现，并逐阶段推送到 `origin/dev`；未打 tag、未发布 npm。当前登记测试数为 **1889**（基线 1831 + Phase A/C 随包回归 56 + Phase D focused 2）。全量 `npm test` 本轮未宣称收口：已有测试路径会触达真实 DingTalk API，安全门禁阻止继续轮询；请把它作为发布前剩余门禁。Phase F 真机/真实 provider 证据仍保持 open，见 `docs/memory/risks.md`。

更新时间：2026-09-25。当前发布线 **v0.12.0（1831 tests 全绿，已并入 `main` 并 tag `v0.12.0`；真机 DSH 宿主 `0.1.7-rc.2` 视觉/交互验证与 `alpha.1` 兼容性冒烟均已通过）**：Native Control Surface——把可编辑出站态从「Standalone Admin 生命周期耦合」迁移为产品/运行时态。canonical 出站键 `channel:<type>:outbound` 始终读取、与 `admin.enabled` 无关；旧 `admin:channel:<type>:outbound` 与 `<type>:account` 仅在 Admin 开启时作兼容读取（Admin 关闭不得复活，§6 兼容红线）。单一运行时权威 `OutboundSource`：notifier/路由/工具/Control Surface 一律按操作时快照读取，出站保存热生效、不重建 notifier、不重启。DSH Connection RPC `/dsh-notifier`（`surface.home`/`surface.wait`、`channels.list`/`get`/`save`/`test`、`tasks.list`、`questions.settle`、`activity.list`、`standalone.createLaunch`）；一次性 Advanced Console 启动票据经 `POST /api/auth/exchange-ticket` 交换，绝不下发长期 Bearer。投影含 Health/Activity/Channels/Tasks/Questions（Activity 脱敏；secret 字段只回 `configured` 不回明文）；`Surface Revision` 负责唤醒。前端为 DSH Native（Sidebar/Main + Host React，无 iframe/esbuild/react-dom），Standalone 管理台降级为高级/恢复入口。
...
| 测试 | `npm test` 当前工作线登记 **1889 tests**（Phase A/C/D focused 与静态门禁已通过；全量门禁待发布前在不触达真实 provider 的隔离条件下完成） |
