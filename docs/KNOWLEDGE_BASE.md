...
- 当前发布线：**v0.11.0**（issue/PR 清零 + 宿主对齐 + 生态公共面，包版本字段 `0.11.0`，`dev` 已收口、待并入 `main`）：W0 善后（#26/#31/#32/#33/#36 + Feishu P2P 来源错配 + wps-bot `http:` 收紧）、宿主对齐（P0-A durable image block / P0-B 结构化 `AgentCancelCause` / P1 Qmsg 3.0 + SC3 + peer 兼容矩阵 / P2 host-events 收敛与提问 seam 校准）、公共面（W1 `testing` fake、W2 `types`、W3 `consumer-demo`、W4 `PLUGINS.en.md`、W5 `/sessions`、W6 `/log`）。上一线 v0.10.2（文档清理与收录状态收口，1616 tests）；npm registry `latest` 在 v0.11.0 发布后更新。
- 当前测试：`1816`（1816 pass，v0.11.0 发布基线）。历史 v0.10.1/v0.10.2 为 `1616`、v0.10.0 为 `1605`、v0.9.7 为 `1548`、v0.9.6 为 `1544`、v0.9.5 为 `1531`、v0.9.0 为 `1352`、npm `0.8.6` 契约为 `909`；按版本区分。
