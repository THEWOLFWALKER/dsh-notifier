...
- The public notifier event previously exposed complete notification content and directed public sends bypassed the normal ledger/event sink. A1/A2 are now implemented on the working branch: cross-plugin events are metadata-only and directed sends share the internal audit callback. Remaining A3-A6 work is tracked in `docs/security/PLUGIN_SECURITY_FIX_PLAN.md`.
